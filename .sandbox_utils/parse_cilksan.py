    ...
    for idx, l in enumerate(race_section):
        for t in mem_access_types:
            if l.find(f"{t} ") != -1:
                ...
                if full_path.find(':')==-1:
                    if full_path.strip().endswith('solution.cpp'):
                ...
                #print(info)
                # NOTE(TFK): Previous logic below that is simpler, but incorrect when function names have spaces in them.
                #memory_access_type = info[0]
                #function = info[2]
                #full_path = info[3]
                #file_name = info[3].split(':')[0].split('/')[-1]
                #line_num = info[3].split(':')[1]
                #col_num = info[3].split(':')[2]
