...
public:
    ...
    void construct_from_coo(std::vector<int64_t>& input_row, std::vector<int64_t>& input_col) {
	// Verification code.
	#if 0
    ...
    ProblemInput(std::string inp_name, quicktype::Inputschema& _input, decltype(initialize_graph) init = initialize_graph) {
        ...
        input = _input;
	if (false) {
		std::vector<eidType> rowptr_tmp = std::vector<eidType>{0,2,5,7,10,12};
		std::vector<vidType> col_tmp = std::vector<vidType>{1,4,0,2,3,1,3,1,2,4,0,3};
		printf("%d\n",col_tmp.size());
		rowptr = new eidType[rowptr_tmp.size()];
		col = new vidType[col_tmp.size()];
		std::copy(rowptr_tmp.begin(), rowptr_tmp.end(), rowptr);
		std::copy(col_tmp.begin(), col_tmp.end(), col);
		N=rowptr_tmp.size()-1;
		M=col_tmp.size();
	} else
